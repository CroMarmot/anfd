...
import { __async, __objRest, __spreadValues } from "@nf-internal/chunk-WDMUDEB6";
...
import { assertInInjectionContext, inject as inject2, Injector, ɵResourceImpl as _ResourceImpl, linkedSignal, computed, signal, ɵencapsulateResourceError as _encapsulateResourceError, ɵRuntimeError as _RuntimeError2, InjectionToken as InjectionToken2, ɵperformanceMarkFeature as _performanceMarkFeature, APP_BOOTSTRAP_LISTENER, ApplicationRef, TransferState, makeStateKey, ɵtruncateMiddle as _truncateMiddle, ɵformatRuntimeError as _formatRuntimeError2 } from "@angular/core";
...
var HttpResourceImpl = class extends _ResourceImpl {
    client;
        ...
        super(request, ({ params: request2, abortSignal }) => {
            ...
            sub = this.client.request(request2).subscribe({
                ...
                complete: () => {
                    if (resolve) {
                        send({
                            error: new _RuntimeError2(991, ngDevMode && "Resource completed before producing a value")
                        });
                    }
                    abortSignal.removeEventListener("abort", onAbort);
                }
...
var HTTP_TRANSFER_CACHE_ORIGIN_MAP = new InjectionToken2(ngDevMode ? "HTTP_TRANSFER_CACHE_ORIGIN_MAP" : "");
var BODY = "b";
var HEADERS = "h";
var STATUS = "s";
var STATUS_TEXT = "st";
...
var CACHE_OPTIONS = new InjectionToken2(ngDevMode ? "HTTP_TRANSFER_STATE_CACHE_OPTIONS" : "");
var ALLOWED_METHODS = ["GET", "HEAD"];
function transferCacheInterceptorFn(req, next) {
    const _a = inject2(CACHE_OPTIONS), { isCacheActive } = _a, globalOptions = __objRest(_a, ["isCacheActive"]);
    ...
    if (typeof ngServerMode !== "undefined" && !ngServerMode && originMap) {
        throw new _RuntimeError2(2803, ngDevMode && "Angular detected that the `HTTP_TRANSFER_CACHE_ORIGIN_MAP` token is configured and present in the client side code. Please ensure that this token is only provided in the server code of the application.");
    }
        ...
        get(target, prop) {
            ...
            return headerName => {
                ...
                if (!headersToInclude.includes(headerName) && !warningProduced.has(key)) {
                    ...
                    console.warn(_formatRuntimeError2(2802, `Angular detected that the \`${headerName}\` header is accessed, but the value of the header was not transferred from the server to the client by the HttpTransferCache. To include the value of the \`${headerName}\` header for the \`${truncatedUrl}\` request, use the \`includeHeaders\` list. The \`includeHeaders\` can be defined either on a request level by adding the \`transferCache\` parameter, or on an application level by adding the \`httpCacheTransfer.includeHeaders\` argument to the \`provideClientHydration()\` call. `));
                }
...
function verifyMappedOrigin(url) {
    if (new URL(url, "resolve://").pathname !== "/") {
        throw new _RuntimeError2(2804, `Angular detected a URL with a path segment in the value provided for the \`HTTP_TRANSFER_CACHE_ORIGIN_MAP\` token: ${url}. The map should only contain origins without any other segments.`);
    }
}
